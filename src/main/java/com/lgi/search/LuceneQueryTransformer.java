...
public class LuceneQueryTransformer {
	private static final Logger logger = Logger
			.getLogger(LuceneQueryTransformer.class.getName());
	private final Analyzer analyzer;
	private final SearcherManager searcherManager;
	private final ObjectMapper objectMapper = new ObjectMapper();
	private static final Set<String> fieldsToInclude = new HashSet<>();

	private final Transformer transformer;
	static {
		try {
			fieldsToInclude.add("case_author");
			fieldsToInclude.add("case_bench");
			fieldsToInclude.add("case_source");
			fieldsToInclude.add("case_title");

			LogManager.getLogManager()
					.readConfiguration(LuceneQueryTransformer.class
	...
	public LuceneQueryTransformer(Directory indexDir) throws Exception {
		...
		InputStream xsltInputStream = getClass().getResourceAsStream("/query-transform.xsl");
		if (xsltInputStream != null) {
		    StreamSource xslt = new StreamSource(xsltInputStream);
		    this.transformer = tf.newTransformer(xslt);
			this.transformer.setOutputProperty(OutputKeys.OMIT_XML_DECLARATION,
					"yes");
		    // Continue with your transformation logic
		} else {
		    throw new Exception("Error: XSLT file not found.");
		}
	}
	...
	public Query transformQuery(String userInput)
			throws IOException, ParseException {
		// Transform user input with XSLT stylesheet
		StringWriter result = new StringWriter();
		try {
			transformer.transform(new StreamSource(new StringReader(userInput)),
		...
		QueryParser parser = new QueryParser("case_judgement", analyzer);
		...
		try {
			...
			for (ScoreDoc hit : hits.scoreDocs) {
				...
				for (IndexableField field : doc.getFields()) {
					if (fieldsToInclude.contains(field.name())) {
		                // Put the field and its value into the map
			...
			try {
				// Convert documentMap to JSON string and append to the
				// StringBuilder
				stringBuilder
						.append(objectMapper.writeValueAsString(jsonArray));
	...
	public static void main(String[] args) {
		try {
			...
			Directory indexDirectory = FSDirectory
					.open(Paths.get("indexingData"));
			// Create the SearcherManager with a SearcherFactory
			LuceneQueryTransformer lqt = new LuceneQueryTransformer(
					indexDirectory);
			String xmlString = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
					+ "<query><userInput>cases related to murder</userInput></query>";
